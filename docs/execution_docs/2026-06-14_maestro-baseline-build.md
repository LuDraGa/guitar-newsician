...
|---|---|
| **Date** | 2026-06-14 |
| **Status** | ✅ A+B+C committed (`9659032`, 2026-06-15) — `/app/maestro` is wired end-to-end to the local baseline agent (Next `/api/maestro/*` → `MAESTRO_AGENT_URL`): pick a song, build/load its fact pack, chat with evidence + an inspectable trace. C4 tests deferred by decision. **Next: scope the Maestro coach vertical slices (Phase D) against the PRD.** |
...
| **COMMIT working base agent** | ✅ Done — `9659032` | "Stand up Maestro's baseline coach agent end-to-end on real song data" (34 files). typecheck + lint clean; agent `/health` + `/fact-pack/{seed}` smoke-tested (Ab/Bb conflict, 110bpm, 10 sections, 10 stems). This doc's status bump is the only post-commit working-tree change. |
...
- **UI iteration — DONE (2026-06-15, post-commit):** `/app/maestro` reworked to a **single-viewport, full-width** workbench (AppShell now gives `/app/maestro` the Studio fixed-height wrapper; each of the 3 columns scrolls internally, no document scroll). Added a **provider + model selector** (OpenAI; `gpt-5.4-nano/mini`, `gpt-5.4`, `gpt-5.5`, `gpt-5.5-pro` shown — `gpt-5.4-pro` hidden — **default `gpt-5.4-nano`** to save credits). This needed agent plumbing: `/chat` now takes `model`, `invoke_agent` resolves+validates it (`openai/*` allowlist, `ModelNotAllowed`→400) and **caches per `(song, model)`**; the seam + Next route pass it through. Added **song search** (title/artist filter) and a **Runtime rail → slide-over drawer**: the rail lists the active turn's `trace.actions` (tool/subagent/final) + `usage_calls`; clicking a row opens a dismissible right-side drawer (scrim + Esc + ✕) that **overlays** rather than reflows. Verified live on a throwaway `:8010` instance: `gpt-5.4-nano` answered the key question (Ab vs Bb) for **$0.0012** (vs ~$0.05 on gpt-5.5), trace shape matches the rail, invalid model → 400.
- **UI iteration 2 — DONE (2026-06-15):** (1) **Model selector** is now a custom `Popover` dropdown — collapsed shows only the model name, the open list adds the cheap/fast hint + the raw id (native `<select>` can't differ collapsed vs option text). (2) **Per-song conversation history**: chats persist to **localStorage** (`maestro:conversations:v1`), a "History" menu in the chat header lists a song's past chats to load + continue, plus a "New" button; selecting a song restores its latest chat. Persistence lives in the send handler (not an effect) to satisfy `react-hooks/set-state-in-effect`; impure `Date.now()/randomUUID` are wrapped in module helpers for `react-hooks/purity`. *(localStorage = per-browser, not durable/cross-device; upgrade to a DB table later if needed.)* (3) **Tools panel** under Runtime: new agent **`GET /tools`** introspects the 7 bounded tools (name/docstring/typed params); clicking a tool opens the same slide-over drawer (now also renders a `description`). Verified live: `/tools` returns all 7 with params + descriptions.
- **⚠️ Runtime gotcha (2026-06-15):** the POC **"Maestro Debug Agent"** was found running on `:8000`. The Next app defaults `MAESTRO_AGENT_URL` to `:8000`, so `/app/maestro` would hit the POC (wrong routes → 404) unless **our** WereCode agent (`cd maestro && uv run uvicorn maestro_agent.app:app --port 8000`) is the thing on `:8000`. Stop the POC, or point `MAESTRO_AGENT_URL` at our instance's port.
