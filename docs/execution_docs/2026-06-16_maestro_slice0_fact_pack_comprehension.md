...
|---|---|
| **Date** | 2026-06-16 |
| **Status** | 🟢 Building — data-core (0.1 / 0.2 / 0.4 + `get_stems`) + **0.2b** (pitch-class content) + **Fact Pack Freshness** committed (`e3b9647`); **0.3** (retrieval discipline) committed + live-verified, **34/34 pytest green**. Next: 0.5 overview. |
...
| Tests (Seam A/B) — data-core + 0.2b + 0.3 | ✅ | `tests/test_slice0_comprehension.py`: identity precedence, MIDI bucketing, roster/detail/activity views, 0.2b PC histogram / dominant-PC ranking / stem PC profile / surfacing, + 0.3 roster-shape assertions (no default dump on `get_midi_tracks`/`get_song_slice`) — **24** here; **34/34** with `test_fact_pack_freshness.py` |
| Try-it (stage prompts) | 🟡 | live + inspection-based. **0.3 ✅ verified** (2026-06-17): "what key… separate for diff instruments" → trace = `get_key` + `get_stems` (roster), **no `get_song_slice`/`get_midi_tracks` 10-stem dump**, honest "one global key, can drill a stem" answer. Data-core/0.2b still to re-confirm on the guitar. |
