...
| **Comprehension Graph store** (#2) | `maestro_agent.comprehension_graph` + `werecode.maestro_comprehension_graph` (RLS'd) | the durable song-knowledge substrate: brief nodes persist keyed `(song, node_type, region_key, pack_key, graph_version)`; a warm region recalls with NO second judgment pass (`source: graph_recall`, ~31× cheaper); invalidation = key miss on pack rebuild (history, never deletions). `ensure_current`/`query` is the composition seam capabilities build on. |
| **`drill` capability — composed on the graph** (#4) | `maestro_agent.drill` + the `drill_region` tool | the second coaching verb — and the proof the substrate compounds (story 19): comprehension comes ONLY via `ensure_current` (a cold region populates the brief node first; no parallel rollup path exists, asserted structurally), then a pure practice plan (loop window, 60/75/90/100% tempo ladder, guitar-first focus, flag/hedge cautions) + ONE `gpt-5.5` drill judgment (`drill-judgment` in the trace). The drill node is ephemeral but node-shaped — persisting it is deferred fog (S3). |
| **Earned harness — grounding + router + evaluator** (#5) | `maestro_agent.harness` | all pure, zero LLM calls, extracted from OBSERVED duplication/defects: the shared judgment runner (`run_judgment` — the brief↔drill try/parse/degrade mirror both docstrings named); dispatch-level `provenance` on every tool response (`ground` — by construction, not author discipline; deterministic, stamped after persistence); the verb router (`route_message` classifies brief/drill/freeform, region grounded in the pack's real section vocabulary, nudge directive on the per-turn message — the cached prefix untouched — with `trace.router.followed` making any mis-route measurable); the evaluator stub (`evaluate_node` warning flags on every returned node — incl. `unknown_section_index`, the #4 live defect class — never blocking, never persisted). |
...
|---|---|---|---|
| **Routing** | pick the right specialist/path for a request | `task` + subagent descriptions | **Removed at #17** (soft routing fired 0/10 on both nano and gpt-5.5). If a future verb needs dispatch (S3 orchestrator–workers), design purpose-built workers with *hard* routing there — don't resurrect the chat roster. **Distinct: #5's VERB router (2026-07-14)** — pure-Python brief/drill/freeform classification at the harness seam, nudging (never forcing) the loop; not `task`-tool dispatch. |
| **Chaining** (prompt-chaining) | fixed multi-step pipeline: comprehend → select → brief → check | a LangGraph sequence / staged prompts | **First chain wired at #1 (2026-07-12):** the brief verb's select → rollup → judge steps are code-sequenced inside `brief_region` (plain Python, not a LangGraph sequence — right-sized for two steps). **The "check" step landed at #5 (2026-07-14):** `evaluate_node` flags a shaky node at the tool seam — the chain is now select → rollup → judge → check. |
