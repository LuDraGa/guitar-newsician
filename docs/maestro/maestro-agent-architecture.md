...
| `create_agent_runner` / `_make_tools` / `_make_subagents` / `_register_maestro_profile` (excluded file tools, general-purpose subagent disabled, `task` override) | The **template for live capability agents**. The specialist-roster half did *not* survive contact with live evidence: delegation fired 0/10 on both nano and gpt-5.5, so #17 (2026-07-12) removed the ported roster and disabled the general-purpose subagent via a `litellm` harness profile — no `task` tool ships. Workers return, purpose-built, only inside an orchestration verb that needs them (S3+). |
| `_agent_trace` (tool calls, model, usage) | The **observability** to keep — essential for the dev inspection feature flag |
